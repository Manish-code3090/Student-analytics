...
exports.createMembership = async (req, res, next) => {
  try {
    ...
    if (callerRole !== 'admin') {
      return res.status(403).json({ message: 'Only admins can create a membership' });
    }
    ...
    if (batch && role !== 'student') {
      return res.status(400).json({ message: 'Only student memberships can have a batch' });
    }

    const existing = await Membership.findOne({ phone, center: centerId });
    if (existing) {
      return res.status(409).json({ message: 'A membership already exists for this phone in this center' });
    }
    ...
    const status = user && (role !== 'student' || batchDoc) ? 'active' : 'pending';
...
/**
 * GET /memberships?role=student&batch=<id>&status=active
...
exports.getMembershipById = async (req, res, next) => {
  try {
    ...
    if (!isSelf && !['admin', 'teacher'].includes(role)) {
      return res.status(403).json({ message: 'Not authorized to view this membership' });
    }

    const membership = await Membership.findById(id).populate('user', 'name phone email').populate('batch', 'name');
...
exports.updateMembership = async (req, res, next) => {
  try {
    const { role: callerRole, centerId } = req.auth;
    if (callerRole !== 'admin') {
      return res.status(403).json({ message: 'Only admins can update a membership' });
    }
...
exports.deleteMembership = async (req, res, next) => {
  try {
    ...
    if (role !== 'admin') {
      return res.status(403).json({ message: 'Only admins can delete a membership' });
    }
    ...
    if (membership.role === 'student') {
      ...
      const resultCount = await Result.countDocuments({ studentMembership: membership._id });
      if (resultCount > 0) {
